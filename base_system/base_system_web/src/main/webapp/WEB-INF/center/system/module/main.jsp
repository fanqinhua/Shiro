<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<c:set var="ctx" value="${pageContext.request.contextPath}"/>
<%@ taglib prefix="shiro" uri="http://shiro.apache.org/tags" %>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="renderer" content="webkit">
    <meta http-equiv="Cache-Control" content="no-siteapp"/>
    <title>菜单管理</title>
    <meta name="keywords" content="XXX管理系统">
    <meta name="description" content="XXX管理系统">
    <jsp:include page="../../common/commonHeaderCSS.jsp"></jsp:include>
</head>
<body>

<div class="layui-row layui-col-space10 layui-form-item" style="margin: 10px;">
    <div class="layui-form-item">
        <div class="layui-inline">
            <label class="layui-form-label" style="width: 150px">菜单名称</label>
            <div class="layui-input-inline" style="width: 150px;">
                <input type="text" name="cname" id="cName-search" placeholder="输入菜单名称" autocomplete="off"
                       class="layui-input">
            </div>
            <shiro:hasPermission name="module:read">
                <button data-toggle="button" class="btn btn-primary bg-main border-color" type="button" id="btn-search">
                    <i
                            class="fa fa-search mr-5"></i>搜索
                </button>
            </shiro:hasPermission>
            <button data-toggle="button" class="btn btn-primary bg-main border-color" type="button" id="btn-expand">展开
            </button>
            <button data-toggle="button" class="btn btn-primary bg-main border-color" type="button" id="btn-fold">折叠
            </button>
            <button data-toggle="button" class="btn btn-primary bg-main border-color" type="button" id="btn-refresh">
                刷新
            </button>
            <div class="clear"></div>
        </div>
        <table id="moduleTable" class="layui-table" lay-filter="moduleFilter"></table>
    </div>
</div>
<jsp:include page="../../common/commonFooterJS.jsp"></jsp:include>
<script>
    var ctx = '${ctx}';
</script>
<script>
    function complain1(d) {//操作中显示的内容
        var add = '';
        var edit = '';
        var del = '';
        if (d.cisLeaf == '1') {
            <shiro:hasPermission name="module:add">
            add = '<a class="operation" lay-event="add" title="新增"><i class="layui-icon layui-icon-add-1"></i></a>&nbsp;'
            </shiro:hasPermission>
        } else {
            add = '';
        }
        if (d.ipid != 0) {
            <shiro:hasPermission name="module:edit">
            edit = '<a class="operation" lay-event="edit" title="编辑"><i class="layui-icon layui-icon-edit"></i></a>&nbsp;'
            </shiro:hasPermission>
        } else {
            edit = '';
        }
        if (d.cisLeaf != '1') {
            <shiro:hasPermission name="module:del">
            del = '<a class="operation" lay-event="del" title="删除"><i class="layui-icon layui-icon-delete"></i></a>'
            </shiro:hasPermission>
        } else {
            del = '';
        }
        return [add, edit, del,
        ].join('');

    }
    function complain2(d) {//操作中显示的内容
        var opration = '';
        if (d.cisLeaf == '0') {
            <shiro:hasPermission name="module:manager">
            opration = '<a class="layui-btn layui-btn-xs" lay-event="opr" title="功能点">管理功能点</a>&nbsp;'
            </shiro:hasPermission>
        } else {
            opration = '-';
        }
        return [opration].join('');
    }
</script>
<script src="${ctx}/static/system/module/module.js" type="application/javascript"></script>
</body>
</html>
